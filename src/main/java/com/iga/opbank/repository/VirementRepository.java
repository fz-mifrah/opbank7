...
public interface VirementRepository extends JpaRepository<Virement, Long> {
    ...
    @Query(
        value = "select distinct virement from Virement virement left join fetch virement.destinataire left join fetch virement.beneficiaire",
    ...
    @Query("select distinct virement from Virement virement left join fetch virement.destinataire left join fetch virement.beneficiaire")
    List<Virement> findAllWithToOneRelationships();

    @Query(
        "select virement from Virement virement left join fetch virement.destinataire left join fetch virement.beneficiaire where virement.id =:id"
    )
